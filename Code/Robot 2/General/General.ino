...
const int KnockerPin = 9;
const int ClawPinL = 10;
const int ClawPinR = 11;


// Chasis Parameters & Constants
const float WheelBaseDiameter = 22.0;                               // sets the distance between the wheels in cm
const float WheelDiameter = 10.2;                                  // sets the diameter of the wheel
...
float S = 45;                                                   // General movement speed
float StartSpeed = 35;
float FinalSpeed = 15;                                          // End movement speed
int DefaultAcceleration = 1;                                    // Sets default acceleration value
float TS = 15.0;                                             // Turn speed, turn speed for each wheel dependent on team colour
...
int CorrectionSpeed = 5;                                      // sets the speed the the distcorrect function uses for each wheel
float OvershootDist = 11;                                      // sets an overshoot correction factor which reduces any movement by this many CM's
float ss = 0.3;
...
void setup() {
  ...
  ServoClawL.attach(ClawPinL);
  ServoClawR.attach(ClawPinR);
  ServoKnocker.attach(KnockerPin);
