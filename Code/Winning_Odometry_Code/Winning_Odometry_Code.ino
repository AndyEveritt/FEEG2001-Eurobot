...
void loop(){
  ...
  LEDflash();                                                 // Flash LED to indicate loop has initiated
  LEDflash();
  LEDflash();
  ...
  movement(S, 0, 38, 38);                                     // Calls a function that moves the platform with a combined speed, turning speed, and distances for each motor in cm, forward 34cm
  DistCorrection();                                           // calls a function to correct for any under/over movement due to momentum or other causes
  LEDflash();                                                 // calls a function to flash the LED and indicate robot has arrived at the point
  ...
  //Serial.println("5 - 6");
  movement(0.7*S, 0, 40, 40);                                   // forward 40cm                       
  DistCorrection();
  LEDflash(); 
  dispense();
  
  //Serial.println("6");
  movement(0, -TurnMulti*S, -((WheelBaseDiameter/2) * pi * (93.0/180)), ((WheelBaseDiameter/2) * pi * (93.0/180)));                                   // 90 degree anticlockwise turn, 3 degree fudge factor
  DistCorrection();

  //Serial.println("6 - 7");
  movement(0.8*S, 0, 40-1, 40-1);                                   // forward 40cm                       
  DistCorrection();  
  LEDflash();
  ...
  //Serial.println("7 - 8");
  movement(S, 0, 40, 40);                                   // forward 40cm                       
  DistCorrection();
  LEDflash(); 
  dispense();
  
  //Serial.println("8");
  movement(0, -TurnMulti*S, -((WheelBaseDiameter/2) * pi * (42.0/180)), ((WheelBaseDiameter/2) * pi * (42.0/180)));                                   // 40 degree anticlockwise turn, 2 degree fudge factor
  DistCorrection(); 

  //Serial.println("8 - 9");
  movement(S, 0, 61.955, 61.955);                                   // forward 61.955cm                       
  DistCorrection();
  LEDflash();

  //Serial.println("9");
  movement(0, TurnMulti*S, ((WheelBaseDiameter/2) * pi * (40.0/180)), -((WheelBaseDiameter/2) * pi * (40.0/180)));                                   // 40 degree clockwise turn  
  DistCorrection();

  //Serial.println("9 - 10");
  movement(-0.6*S, 0, -18, -18, 1);                                   // reverse 18cm                       
  DistCorrection();
  LEDflash();
  dispense();
  
  //Serial.println("10");
  movement(0, TurnMulti*S, ((WheelBaseDiameter/2) * pi * (92.0/180)), -((WheelBaseDiameter/2) * pi * (92.0/180)));                                   // 90 degree clockwise turn  
  DistCorrection();

  //Serial.println("10 - 11");
  movement(-1.4*ArcMulti*18, -1.5*ArcMulti*WheelBaseDiameter/2, -((3*pi/2)*(18 - (WheelBaseDiameter/2))+4), -((3*pi/2)*(18 + (WheelBaseDiameter/2))+4));                                   // 270 degree clockwise arc radius 26cm travelling backwards, 4cm fudge factor on each wheel
  DistCorrection();                                           // calls a function to correct for any under/over movement due to momentum or other causes
  LEDflash();                                                 // calls a function to flash the LED and indicate robot has arrived at the point

  //Serial.println("11");
  movement(0, TurnMulti*S, ((WheelBaseDiameter/2) * pi * (40.0/180)), -((WheelBaseDiameter/2) * pi * (40.0/180)));                                   // 37.41 degree clockwise turn  
  DistCorrection();

  //Serial.println("11 - 12");
  movement(S, 0, 35.928, 35.928);                                   // forward 35.928cm                       
  DistCorrection();
  LEDflash();

  //Serial.println("12 - 13");
  movement(S, 0, 42.802, 42.802);                                   // forward 42.802cm                       
  DistCorrection();
  LEDflash();
}
...
void movement (int SpeedValue, int TurnValue, float Distance_1_CM, float Distance_2_CM, int Acceleration, boolean VariableSpeed){                                               // This function moves the platform with a set speed and turn value for each wheel until they have moved the defined distance set in cm
  ...
  Last_Distance_2 = Distance_2_CM;                                       // Sets the distance for wheel 2 as a global variable so it can be used by the DistCorrection function
    ...
    while(abs(e1) < abs(Distance_1_CM) && abs(e2) < abs(Distance_2_CM)){     // If statement to check the status of the traveled distance, absolutes used incase the motors are in reverse
    ...
    if(VariableSpeed == true && (e1/Distance_1_CM) > 0.5){                                           // Checks whether speed needs to be varibale or constant
      Speed = (1-(1-(10.0/S))*(e1/Distance_1_CM))*SpeedValue;                       // Multiplies the SpeedValue so it starts at 100% speed at the start of the movement, after 50% of the movement it decreases linearly and ends at speed 10
      Turn = (1-(1-(10.0/S))*(e1/Distance_1_CM))*TurnValue;                         // Multiplies the TurnValue so it starts at 100% speed at the start of the movement, after 50% of the movement it decreases linearly and ends at speed 10
    }
    else{
...
void DistCorrection(){                                        // Corrects any over/undermovement due to momentum, corrects each wheel independantly to do both distance and angle.
  ...
  if (D1 <= limit || D1 >= limit){                                                     // Only corrects if the difference is larger enough as it will overcorrect if the distance is small
    movement((CorrectionSpeed * Sign1), CorrectionSpeed, D1, 10000, 1, false);            // Corrects wheel 1 so the encoder reads what it should, distance 2 is arbitrary but needs to be high so it does not stop the movement function before wheel 1 has adjusted
  }
  
  if (D2 <= limit || D2 >= limit){                                                     // Only corrects if the difference is larger enough as it will overcorrect if the distance is small
    movement((CorrectionSpeed * Sign2), -CorrectionSpeed, 10000, D2, 1, false);           // Corrects wheel 2 so the encoder reads what it should, distance 1 is arbitrary but needs to be high so it does not stop the movement function before wheel 2 has adjusted
  }
...
void dispense() {
  myservo.write(110);                                       // Moves the servo to 110 degrees
  delay(800);                                               // Ensures full movement has occurred
  myservo.write(5);                                         // Returns servo to 5 degrees
  delay(50);                                               // Ensures M&M has settled
}
