...
float WheelBaseDiameter = 31.4;                               // sets the distance between the wheels in cm
...
float S = 70;                                                   // General movement speed
float StartSpeed = 20;
...
void movement (int SpeedValue, float TurnAngle, float Distance, int Acceleration, boolean VariableSpeed, boolean Correction){                                               // This function moves the platform with a set speed and turn value for each wheel until they have moved the defined distance set in cm
  ...
  while(abs(e1) < abs(Distance_1_CM)- oscf || abs(e2) < abs(Distance_2_CM)-oscf){     // If statement to check the status of the traveled distance, absolutes used incase the motors are in reverse
    ...
    else if(VariableSpeed == true && (e1/Distance_1_CM) < ss){                                           // Checks whether speed needs to be varibale or constant
      float c1 = (SpeedValue-StartSpeed)/(pow((ss), 2));
      float c3 = StartSpeed;
      float PT1 = e1/Distance_1_CM;
      Speed1 =  (c1*pow(PT1, 2) + c3) * (Speed1/abs(Speed1));
    }
    ...
    else if(VariableSpeed == true && (e2/Distance_2_CM) < ss){                                           // Checks whether speed needs to be varibale or constant
      float c1 = (SpeedValue-StartSpeed)/(pow((ss), 2));
      float c3 = StartSpeed;
      float PT1 = e2/Distance_2_CM;
      Speed2 =  (c1*pow(PT1, 2) + c3) * (Speed2/abs(Speed2));
    }
